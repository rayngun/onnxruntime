...
if [ "$os_major_version" -gt 7 ]; then
    PACKAGE_MANAGER="dnf"
    $PACKAGE_MANAGER install -y which redhat-lsb-core expat-devel tar unzip zlib-devel make bzip2 bzip2-devel perl-IPC-Cmd openssl-devel wget --nogpgcheck
else
    PACKAGE_MANAGER="yum"
...
$PACKAGE_MANAGER install -y msopenjdk-11 graphviz --nogpgcheck
