  ...
  try {
    ...
    if (global_context.is_wholly_supported_graph) {  // Full graph is supported
      ...
      if (is_ep_ctx_graph_) {
        ...
        std::cout << " Before_serialise_onnx_model_workingset_size = " << before_serialise_onnx_model_workingset_size << std::endl;

        // const std::string model = model_proto.SerializeAsString();
        size_t after_serialise_onnx_model_workingset_size = GetPeakWorkingSetSize();
        exe_network_ = global_context_.ie_core.CompileModel(model,
                                                            hw_target,
                                                            device_config,
                                                            subgraph_context_.subgraph_name);
        size_t after_ov_compile_workingset_size = GetPeakWorkingSetSize();

        std::cout << " After_serialise_onnx_model_workingset_size = " << after_serialise_onnx_model_workingset_size << std::endl;
        std::cout << " After_OV_compile_workingset_size = " << after_ov_compile_workingset_size << std::endl;
