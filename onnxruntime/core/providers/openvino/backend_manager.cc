...
void BackendManager::GetModelProtoFromFusedNode(const onnxruntime::Node& fused_node,
  ...
  if (global_context_.device_type.find("NPU") != std::string::npos &&
    ...
    LOGS_DEFAULT(INFO) << "[OpenVINO-EP] QDQ optimization pass status: 1";
    {
      ...
      std::cout << "Peak working set - before proto creation in GetModelProtoFromFusedNode = " << onnxruntime::openvino_ep::backend_utils::GetPeakWorkingSetSize() << std::endl;
      std::cout << "Current working set - before proto creation in GetModelProtoFromFusedNode = " << onnxruntime::openvino_ep::backend_utils::GetWorkingSetSize() << "\n" <<std::endl;
      ...
      std::cout << "Peak working set - after proto creation in GetModelProtoFromFusedNode = " << onnxruntime::openvino_ep::backend_utils::GetPeakWorkingSetSize() << std::endl;
      std::cout << "Current working set - after proto creation in GetModelProtoFromFusedNode = " << onnxruntime::openvino_ep::backend_utils::GetWorkingSetSize() << "\n" <<std::endl;

      model_str = model_proto->SerializeAsString();

      std::cout << "Peak working set - after model creation in GetModelProtoFromFusedNode = " << onnxruntime::openvino_ep::backend_utils::GetPeakWorkingSetSize() << std::endl;
      std::cout << "Current working set - after model creation in GetModelProtoFromFusedNode = " << onnxruntime::openvino_ep::backend_utils::GetWorkingSetSize() << "\n" <<std::endl;

      auto x = model_proto.release();
      delete x;

      std::cout << "Peak working set - after releasing model proto in GetModelProtoFromFusedNode = " << onnxruntime::openvino_ep::backend_utils::GetPeakWorkingSetSize() << std::endl;
      std::cout << "Current working set - after releasing model proto in GetModelProtoFromFusedNode = " << onnxruntime::openvino_ep::backend_utils::GetWorkingSetSize() << "\n" <<std::endl;
